...
Autogenerated controller (`/firebase` prefix)

- **GET /api/v1/firebase/topics_overview?deactivatedSince=:ISOTimestamp**
  - Returns the names of all active topics and all topics that have been deactivated since the given timestamp
  - Response: `{ "activeTopics": string[], "deactivatedTopics": string[] }`
- endpoints under the **/api/v1/firebase/topics** path
  - all except **DELETE** - same as autoenerated controllers (see below)
  - firebase topics may not be deleted - they can only be deactivated
  - topic name must follow the pattern `/^[a-zA-Z0-9-_.~%]{1,48}$/`
- **POST /api/v1/firebase/broadcast**
  - Broadcasts a push notification to all users subscribed to the given topic (or any of the given topics if multiple)
  - Request body:
    ```json
    {
      "notification": {
...
Data about any notification sent (and whether successful or not as well) is available under the Notification controller.

##### Notification

Autogenerated controller (`/notification` prefix)

- **GET /api/v1/notification?include=<topics separated by comma>&exclude=<topics separated by comma>**
  - Get all notifications for the given topics (include → whereIn, exclude → whereNotIn)
    The following request `GET /api/v1/notification?include=topic1,topic2&exclude=topic3` will return all notifications that have been sent to `topic1` (among others), and, at the same time, have been sent to neither `topic2` nor `topic3`.
    Both `include` and `exclude` params may be omitted - if so, all topics will be returned.

- **GET /api/v1/notification/:id**
  - works the same as the autogenerated controller endpoint

##### Permissions

All endpoints in this controller require the `solvro_admin` role!

- **POST /api/v1/permissions/allow**
  - Adds a new user permission for a model or model instance.
  - See `permissionChangeValidator` in `/app/validators/permissions.ts` for details about the expected request body
- **POST /api/v1/permissions/revoke**
  - Revokes a permission from the user.
  - Exact same request body as for `allow`
- **GET /api/v1/permissions/list?userId=<number>**
  - Returns the user's roles and permissions.
