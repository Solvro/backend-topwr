...
- **POST /api/v1/**
  ...
  - Request body:
    ...
    {
      "include": ["topic1"],
      "exclude": ["topic2", "topic3"]
    }
    ```
    The request body above will return all notifications that have been sent to `topic1` (among others), and, at the same time, have been sent to neither `topic2` nor `topic3`.
    Both `include` and `exclude` fields may be omitted.
