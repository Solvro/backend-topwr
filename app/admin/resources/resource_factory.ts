...
export interface OwnedRelationDefinition {
  ...
  relationDefinition:
    ...
    | ManyToManyOwnedRelationDefinition;
}

/**
 *  If you don't provide the plural snake_case and camelCase names yourself, they will be autogenerated.
 *  See {@link anyCaseToPlural_snake_case} and {@link anyCaseToPlural_camelCase}
 */
...
/**
 *  If you don't provide the plural snake_case and singular camelCase names yourself, they will be autogenerated.
 *  See {@link anyCaseToPlural_snake_case} and {@link anyCaseToSingular_camelCase}
 */
export interface OneToManyTargetRelationDefinition {
  ownerModel: LucidModel;
  ownerModelPlural_snake_case?: string;
...
export class ResourceFactory {
  ...
  private static addOwnedRelations(
    ...
    const relationDefinitions: RelationsFeatureOptions =
      resourceInfo.ownedRelations.reduce(
        (
          options: RelationsFeatureOptions,
          { displayLabel, relationDefinition }: OwnedRelationDefinition,
        ) => {
          options[displayLabel] = isManyToMany(relationDefinition)
            ? this.addOwnedManyToManyRelation(
                resourceInfo.forModel,
                relationDefinition,
              )
            : this.addOwnedOneToManyRelation(
                resourceInfo.forModel,
                relationDefinition,
              );
          return options;
  ...
  private static addOwnedOneToManyRelation(
    parentModel: LucidModel,
    relationDefinition: OneToManyRelationDefinition,
  ): OneToManyRelationOptions {
    return {
  ...
  private static addTargetRelations(
    ...
    this.addRelationReferenceProperty(
      resource,
      resourceInfo.forModel,
      resourceInfo.targetedByModels,
    );
  }

  private static addRelationReferenceProperty(
    resource: ResourceWithProperties,
    childModel: LucidModel,
    owners: OneToManyTargetRelationDefinition[],
  ) {
    owners.forEach((owner) => {
      const relationKey = getOneToManyRelationForeignKey(
        childModel,
        owner.ownerModelSingular_camelCase ??
          anyCaseToSingular_camelCase(owner.ownerModel),
      );
      resource.options.properties[relationKey] = {
        reference:
          owner.ownerModelPlural_snake_case ??
          anyCaseToPlural_snake_case(owner.ownerModel),
      };
    });
  }
