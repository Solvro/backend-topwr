  ...
  public updatePivotValidator(relationName: string): AnyValidator {
    return this.#relationUpdatePivotValidators.getOrInsertWith(
      ...
      () => {
        ...
        const declaredColumnTypes = relation.options.meta.declaredColumnTypes;
        const queryFields = Object.fromEntries(
          Object.entries(declaredColumnTypes)
            .filter(([, field]) => !field.autoGenerated && field.detachFilter)
            .map(([name, field]) => [name, field.validator]),
        );
        ...
        return vine.compile(
          vine.object({
            query: vine.object(queryFields),
            update: vine.object(updateFields),
          }),
        );
      },
    );
