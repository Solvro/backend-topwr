      ...
      switch (options.type) {
        case "string": {
          if (options.meta.optional === true) {
            options.meta.validator = vine.string().minLength(1).optional();
          } else {
            options.meta.validator = vine.string().minLength(1);
          }
          break;
        }
        case "number": {
          if (options.meta.optional === true) {
            options.meta.validator = vine.number().optional();
          } else {
            options.meta.validator = vine.number();
          }
          break;
        }
        case "boolean": {
          if (options.meta.optional === true) {
            options.meta.validator = vine.boolean().optional();
          } else {
            options.meta.validator = vine.boolean();
          }
          break;
        }
        case "integer": {
          if (options.meta.optional === true) {
            options.meta.validator = vine.number().withoutDecimals().optional();
          } else {
            options.meta.validator = vine.number().withoutDecimals();
          }
          break;
        }
        case "uuid": {
          if (options.meta.optional === true) {
            options.meta.validator = vine
              .string()
              .uuid({ version: [4] })
              .optional();
          } else {
            options.meta.validator = vine.string().uuid({ version: [4] });
          }
          break;
        }
      }
