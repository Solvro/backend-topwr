...
export function validateColumnDef(
  ...
  return (
    ...
    // meta.typing.hasDefault
    "hasDefault" in meta.typing &&
    typeof meta.typing.hasDefault === "boolean" &&
    // meta.typing.prohibitEdit
    "prohibitEdit" in meta.typing &&
    typeof meta.typing.prohibitEdit === "boolean"
  );
}
...
type SharedTypedColumnOptions = Partial<ColumnDef> & {
  ...
  optional?: boolean;
  /**
   * defaults to the computed value of autoGenerated - if true, this column can be undefined in the create request
   * optional does not imply hasDefault - hasDefault should be set only if a non-null default value will be set by the database
   */
  hasDefault?: boolean;
  /**
  ...
  /**
   * defaults to false
   * if true, this field cannot be specified in update requests
   */
  prohibitEdit?: boolean;
};
...
function computeTyping(
  ...
  if (options.foreignKeyOf !== undefined) {
    ...
    return {
      ...
      prohibitEdit: options.prohibitEdit ?? false,
...
function dateTypedColumn(
  ...
  options.meta.typing.hasDefault =
    options.hasDefault ?? options.meta.typing.autoGenerated;
  options.meta.typing.prohibitEdit = options.prohibitEdit ?? false;
...
export function typedManyToMany<Related extends LucidModel>(
  ...
  const meta = (options.meta ?? { declaredColumnTypes: {} }) as {
    declaredColumnTypes: Record<string, RelationColumnDef>;
  };
  options.meta = meta;
    ...
    Object.entries(options.pivotColumns).map(([name, opts]) => {
      ...
      if (declaredType === "enum") {
        ...
        return [
          ...
          {
            type: declaredType,
            allowedValues: Object.values(opts.type),
            validator,
            optional: opts.optional ?? false,
            hasDefault: opts.hasDefault ?? opts.autoGenerated ?? false,
  ...
  if (options.pivotTimestamps === true) {
    meta.declaredColumnTypes.createdAt = {
      type: "DateTime",
      validator: vine.luxonDateTime(),
      optional: false,
      hasDefault: true,
      autoGenerated: true,
      detachFilter: false,
    };
    meta.declaredColumnTypes.updatedAt = {
      type: "DateTime",
      validator: vine.luxonDateTime(),
      optional: false,
      hasDefault: true,
