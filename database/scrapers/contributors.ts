      ...
      this.directusSchemas.map((schema) =>
        this.semaphore.runTask(async () => {
          ...
          try {
            ...
            throw new Error(`Failed to deserialize ${schema} response`, {
              cause: e,
            });
          }
        }),
    ...
    const imageMap = new Map(
      await Promise.all(
        Array.from(imageIds).map((id) =>
          this.semaphore.runTask(async () => {
            const detailsResponse = await this.wrappedFetch(
              `https://admin.topwr.solvro.pl/files/${id}?fields=filename_disk`,
              `details for file ${id}`,
            );
            let detailsJSON;
            try {
              detailsJSON = (await detailsResponse.json()) as {
                data: { filename_disk: string };
              };
            } catch (e) {
              throw new Error(`Failed to deserialize details for file ${id}`, {
                cause: e,
              });
            }
            const extension = detailsJSON.data.filename_disk
              .split(".")
              .pop()
              ?.toLowerCase();
            const fileResponse = await this.wrappedFetch(
